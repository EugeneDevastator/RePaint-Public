...
logfile.write("\n third init block complete-1");logfile.close();logfile.open(QFile::Append);
...
Label1 = new QLabel("jkl");
   ...
   //LblServIp->setTextInteractionFlags(Qt::TextSelectableByMouse);
    ...
    logfile.write("\n layout block complete");    logfile.close();    logfile.open(QFile::Append);
...
logfile.write("\n main layout block complete");    logfile.close();    logfile.open(QFile::Append);
   ...
   logfile.write("\n dialog init block complete");   logfile.close();   logfile.open(QFile::Append);
...
// used for receiving actions from network!
   ...
   // LOGM->AddAct(act);
if (!ARTM->isRunning()) ARTM->start(QThread::NormalPriority);
...
KBLINK->UnPressAll();
    ...
    else if (KBLINK->KBstate[ekSatP]==true)  BControls->CtlSat->SetValF(BControls->CtlSat->GetValue()+0.01);
    else if (KBLINK->KBstate[ekSatM]==true)  BControls->CtlSat->SetValF(BControls->CtlSat->GetValue()-0.01);
...
void MHPwindow::GetColor(QColor col){  //get color from various sources ike canvas cpicker etc.
...
void MHPwindow::SnapImage(){
    ...
    for (int i=Num.length();i<3;i++) Num.prepend("0");
    //QFile ftest;
...
void MHPwindow::SaveImg(){
    if (!WorkFilePath.isEmpty()) MainImage->SaveImg(WorkFilePath);
...
w->show();
        ...
        foreach (QWidget *w,AllPanels){
            ...
            if ((newx+w->width()) > this->width()) newx= this->width()-w->width();
            if (newy+w->height()>this->height()) newy= this->height()-w->height();
             w->setWindowFlags(Qt::CustomizeWindowHint|Qt::WindowMinimizeButtonHint);
             w->hide();
             w->setGeometry(newx,newy,w->geometry().width(),w->geometry().height());
             w->show();
