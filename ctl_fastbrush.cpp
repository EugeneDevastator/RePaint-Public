...
ctl_FastBrush::ctl_FastBrush(QWidget *parent) :
    QWidget(parent)
{
//g_Brush =new d_Brush;
ZoomK=new float;
...
void ctl_FastBrush::mousePressEvent(QMouseEvent *event){
    ...
    float d30=M_PI*30/180;
    if (fabs(((ang+M_PI)*180/M_PI)-g_Brush->resangle)<5) mmode =4;
    else if ((ang>d30)&(ang<d30*5)) mmode=1;
    else if ((ang<d30)&(ang>(-M_PI*0.5))) mmode=2;
    else if ((ang>d30*5)|(ang<(-M_PI*0.5))) mmode=3;
...
//    float radrel=g_Brush->rad_out/g_Brush->rad_in;
    ...
    if ((ang>d30)&(ang<d30*5)) mode=1;
    if ((ang<d30)&(ang>(-M_PI*0.5))) mode=2;
    if ((ang>d30*5)|(ang<(-M_PI*0.5))) mode=3;

    if (mmode==1) {  //radius
        float rel=g_Brush->rad_in/g_Brush->rad_out;
        if (!((ang>d30)&(ang<d30*5))) (rad=Discrete(rad,10));
       // if (mode !=1) rad=0;
        g_Brush->rad_out=rad;
    ...
    if (mmode==2) {  // rad rel
        ...
        if (mode !=2) g_Brush->rad_in=0;

        if (g_Brush->rad_in<7) g_Brush->rad_in=0;
      float rel=g_Brush->rad_in/g_Brush->rad_out;
      if (rel>0.99) rel=0.98;
    ...
    if (mmode==4) {
        ...
        if ((absrad>anglerad+32)|(rad<20)) ang=Discrete(ang,45*0.5);
