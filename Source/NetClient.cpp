...
void NetClient::G_ExecData(stNetHead HEAD, QByteArray data,NetSocket *dest){
    ...
    if (HEAD.Hid==sdLogin) {             // user is connected and i send him request for authentification;
        emit ReqLogin(DSZstring(data));
        }

    else if (HEAD.Hid==sdLoginS) {               // login success, you get this when you are connected;
    ...
    else if (HEAD.Hid==sdUserAdded){    // user added, everyone gets this when someone connected
        emit LoginS(DSZstring(data));
        }
    ...
    else if (HEAD.Hid==sdUserStat){ // user status
    ...
    else if (HEAD.Hid==sdRoomJoin){ // user status
