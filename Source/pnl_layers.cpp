...
quint16 pnl_Layers::CheckIndex(quint16 idx){
    ...
    if (LayerProps->count()<(idx+1)) idx=LayerProps->count()-1;
    return idx;
}
...
else LayerList->item(0)->setSelected(true);
    return Lyr;
...
void pnl_Layers::AddLayerAt(int before){
  ...
  LayerList->addItem(itmw);
  if (LayerList->selectedItems().count()<1)  LayerList->itemAt(0,0)->setSelected(true);
...
void pnl_Layers::SetLock(qint8 lk){
    locked+=lk;
    if (locked > 0) this->setEnabled(false);
      else
        {
