...
ArtThread::ArtThread(ImageArray *mimg, QObject *parent) : QThread(parent)
{
    ARTM = new ArtMaster();
    MainImage = mimg;
    threadID = 0;
...
void ArtThread::ExecAction(QImage *img, d_Action Act, bool local)
{
    ...
    ap.islocal = local;
    if (!local)
        ActionNetList.append(ap);
    else
        ActionList.append(ap);
...
void ArtThread::ExecActionList(QList<actionpair> APlist)
{
    ...
    */
    ...
    //  ap.img=img;
    //  ap.islocal=local;
    if (!APlist.at(0).islocal)
        ActionNetList.append(APlist);
    else
        ActionList.append(APlist);
    // if (!executing) this->start(QThread::LowestPriority);
    if (!executing)
    {
