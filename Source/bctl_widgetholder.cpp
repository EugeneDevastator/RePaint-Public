...
bctl_WidgetHolder::bctl_WidgetHolder(QWidget *parent) : QWidget(parent)
{
...
QRect bctl_WidgetHolder::AlignWidgets(QRect actor, QWidget *stator)
{
    ...
    if (dtt < snaph)
        newgeo.moveTop(stator->geometry().top());

    // top to bott
    int dtb = abs(actor.top() - stator->geometry().bottom());
    if (dtb < snaph)
        newgeo.moveTop(stator->geometry().bottom());
    ...
    if (dbt < snaph)
        newgeo.moveBottom(stator->geometry().top());
    // if (newgeo!=stator->geometry())
    return newgeo;
    // return 0;
}
