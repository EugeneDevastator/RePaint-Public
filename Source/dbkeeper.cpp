...
void DBKeeper::ReadBase()
{
    ...
    if (SLType == slSMF)
    {
        ...
        bool ok = MainDB.open();
        if (ok)
            emit SendMsg("connected to sqlDB for SMF forums at:" + DBHost);
        else
        {
    ...
    if (lfn.rowCount() > 0)
    {
        ...
        */
        ...
        if (!flist.contains(frec.record(0).value(FMainId).toLongLong()))
        {
            ...
            if (strFlist.length() < 1)
                addqr.addBindValue(QString::number(frec.record(0).value(FMainId).toLongLong()));
            else
                addqr.addBindValue(strFlist + "," + QString::number(frec.record(0).value(FMainId).toLongLong()));
