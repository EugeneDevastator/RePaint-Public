...
float ArtMaster::GenCurveF(float val,float midp){
    ...
    else if (midp>0) return (val*val*val-val)*midp+val;
   else return val;
}
void ArtMaster::GenClamp(QImage *img, float top,float min){
int sal;
//signed int tsal;
if (min>=top) min=top-0.1;
//float mul=1/(top-min);
...
void ArtMaster::DoActionList(QImage *img, dotList* actlist,bool local=true){
    ...
    if (Act->ToolID == eBrush) this->DrawBrush(img,Act->Stroke,Act->Brush);
    else if (Act->ToolID == eSmudge) DrawSmudge(img,Act->Stroke,Act->Brush);
    else if (Act->ToolID == eDisp) DrawDisp(img,Act->Stroke,Act->Brush);
    else if (Act->ToolID == eCont) LvlBrush(img,Act->Stroke,Act->Brush);
    else if (Act->ToolID == eLine) DrawLine(img,Act->Stroke,Act->Brush);
...
void ArtMaster::DrawLine(QImage *img, d_Stroke STRK, d_Brush BRSH){
    ...
    if (dx23<0) xoff=abs(dx23)+wd;
    if (dy23<0) yoff=abs(dy23)+wd;
...
if (qAlpha(cpx[j])>0) {
           ...
           cv=cv/3;
           if (cv>maxm-rng) maxm=(maxm+cv)/2;
           if (cv<minm+rng) minm=(minm+cv)/2;
    //       if (cv>maxm) maxm=cv;
    //       if (cv>minm) minm=cv;
}
               }
...
*/
  ...
  // ------------------- noise cycle -------------
   ...
   if (BRSH.Realb.pipeID==plCFNSR){
    ...
    if (fop<=0) this->GenFocalInv(&Bimg,fabs(fop));
    else if (fop<=1) this->GenFocal(&Bimg,fop);
    if (!((sol==1) & (sol2op==0)))
