...
void NetServer::S_RegisterSock(NetSocket *sock){
    ...
    if (!NameClientList.contains(sock->RegName)){
        ...
        if (NetMode==emServer) {
            ...
            foreach (NetSocket *client,RSockClientList){
                if (client->listIdx != sock->listIdx) G_SendUserStatus(sock,client);
                if (client->listIdx != sock->listIdx) G_SendUserStatus(client,sock);
                }
...
void NetServer::S_DelFromRoom(NetSocket *client){
    if (client->inroom==true) {
        ...
        if (client->Role==srServ) {  //transit server role to first in line
...
*/
     if (NetMode==emServer)
       foreach (NetSocket *sck,RSockClientList) G_SendUserStatus(client,sck);
...
void NetServer::S_Retranslate(stNetHead HEAD,QByteArray fulldata,NetSocket *caller){
    ...
    if (HEAD.Hid==sdFriendSeek) { //friend search;
     if (NetMode==emGServer){
    ...
    if (HEAD.Hid==sdFriendAdd) { //friend search;
      if (NetMode==emGServer){
          QString fname=DSZstring(fulldata);  //(fulldata.right(fulldata.size()-caller->HeadSize));
    ...
    if (HEAD.Hid==sdFriendDel) { //friend delete;
     if (NetMode==emGServer){
    ...
    if (HEAD.Hid==sdRoomJoin) { //join room
    ...
    if (HEAD.Hid==sdRoomPart) {  //room part
...
void NetServer::S_KillClient(NetSocket *caller){
    ...
    if (caller->Registered==true){
        if (NetMode==emGServer) GuidClientList.remove(caller->UGUID);
        ...
        if (NetMode==emServer) { //delete from main chat in case of LAN server.
            foreach (NetSocket *sck,RSockClientList) G_SendData(sdUserDel,SZstring(DelName),sck);
            if (caller->imageowner) RSockClientList[0]->imageowner=true;
                }
