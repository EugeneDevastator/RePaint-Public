...
ImageArray::ImageArray(QWidget *parent) :
    QWidget(parent)
{
...
*/
  ...
  //  if (!Brep)
        ...
        painter.drawLine(BPos2,QPointF(BPos2.x()+(Btilt2.x()*2-1)*Brad2*2,BPos2.y()+(Btilt2.y()*2-1)*Brad2*2));
...
/*
 ...
 //   if (!Brep)
        ...
        painter.drawLine(BPos,QPointF(BPos.x()+(Btilt.x()*2-1)*Brad*2,BPos.y()+(Btilt.y()*2-1)*Brad*2));
        Brad2=Brad;
        BPos2=BPos;
        Btilt2=Btilt;
    ...
    if (MMode==mCrv) {
        rad=(1-BControls->CtlCrv->GetValue())*0.5*g_Brush->rad_out*ZoomK*RadCtl->GetModValue(lastspars)*ScaleCtl->GetModValue(lastspars);
        if (rad < 1) rad = g_Brush->rad_out*ZoomK;
      painter.drawEllipse(BPos,rad,rad);
}
    if (MMode==mPick) {
     //   painter.setRenderHint(QPainter::SmoothPixmapTransform,false);
      ...
      //  painter.setPen(QPen(Qt::SolidLine));
       ...
       painter.setOpacity(0.3);
        painter.drawRect(BPos.x()-Loupesize-5+1,BPos.y()-Loupesize-5+1, ViewLoupe.width()-2,ViewLoupe.height()-2);
...
//    float dir2=AtanXY((strk.pos2.x()-strk.pos3.x()),strk.pos2.y()-strk.pos3.y());
   ...
   // stpars.Pars[csAcc]=(stpars.Pars[csAcc]+lastspars.Pars[csAcc])*0.5;
    ...
    spacing=((SpacingCtl->GetModValue(currspars)*SpacingCtl->GetModValue(currspars))*g_Brush->rad_out*ScaleCtl->GetModValue(currspars)*(RadCtl->GetModValue(currspars)));
    ...
    if (tdist>spacing){
        ...
        float rnflw;
        float rnside;
        float rrang=SpacingJitCtl->GetModValue(currspars)*g_Brush->rad_out*ScaleCtl->GetModValue(currspars)*RadCtl->GetModValue(currspars);
        d_Stroke outstrk;
...
for (int i=0;i<csSTOP;i++) currspars.Pars[i]=1;
  ...
  //  currspars.Pars[csERASER] = 0;
    ...
    float penrot=AtanXY(cxtilt,cytilt);  currspars.Pars[csRot]=RngConv(penrot,-M_PI,M_PI,0,1);
    ...
    QPointF curpos;
    QPointF offpos=QPointF(event->hiResGlobalX(),event->hiResGlobalY());                                                                 // T A B L E T //
    QPointF apos;
    QPoint localLrPos;
    ...
    */
    ...
    if ((MMode==mNone) && (TMode == 1)){
        ...
        if (GlobalKB->KBstate[ekCanvasMove]==true) {
            ...
            PPos=event->pos(); MMode=mZoom; }
        else if (GlobalKB->KBstate[ekFastBrush]==true) {
            PPos=event->pos(); MMode=mRadSize; }
...
void ImageArray::ParseEndPosF(QPointF pos){
    ...
    if (MMode!=mPaint) {
     this->RepaintWidgetRect(QRect(pos.x()-Loupesize-5,pos.y()-Loupesize-5, (qreal)Loupesize,(qreal)Loupesize));
        GenAllThumbs();
     MergeAll();
    }
...
//currspars=InterpolateSpars(currspars,lastspars,0.5);
    ...
    if (MMode==mPan){
        ...
        wasmerged=false;
        d_Stroke strk;
        strk.pos1=CalcPos(pos);                                                                     //  MOVE    POS     PARSER  //
...
uptimer->setInterval(activetime);
 ...
 Drawn=false;
    ...
    else if (GlobalKB->KBstate[ekFastBrush]==true) {PPos=event->pos();
        if (event->button()==Qt::LeftButton) MMode=mRadSize;
        if (event->button()==Qt::RightButton) MMode=mrad2Size;
        if (event->button()==Qt::MiddleButton) MMode=mCrv;
    }
