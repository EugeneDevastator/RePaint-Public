...

{
    ...
    logfile.write("\n brushsizer init BEGIN");  logfile.close(); logfile.open(QFile::Append);
   ...
   logfile.write("\n brushsizer artm+");  logfile.close(); logfile.open(QFile::Append);
   ...
   logfile.write("\n brushsizer init FINISH");  logfile.close(); logfile.open(QFile::Append);
}
...
void BrushSizer::mousePressEvent(QMouseEvent *event){
    ...
    if (event->button()==Qt::RightButton) Mode=2;
    if (event->button()==Qt::MiddleButton) Mode=3;
...
void BrushSizer::Redraw(){
    ...
    if (rad_in<rad_out) grad.setColorAt(1, QColor::fromRgbF(0,0,0,0));
...
rad_in=rad*saverel;
    ...
    offang=AtanXY(ax,ay);
    if (rad>hsize*1.6) offang=Discrete(offang,M_PI_4*0.5);


    g_Brush->resangle=rad2deg(offang);
}
if (rad_in>rad_out) rad_in=rad_out;
