...
bctl_DblSlider::bctl_DblSlider(QWidget *parent) :
    bctl_ImageWidget(parent)
{
    ActivePick=-1;
    Range = 32767;
    DsRange=0.25;
...
void bctl_DblSlider::Redraw(){
    ...
    if (orient==0) {
        ...
        if (jitter>0)  painter.drawRect(0,0,this->width()*jitter,7);
...
painter.setPen(Qt::NoPen);
    ...
    else
    {
        ...
        painter.setPen(Qt::blue);
        painter.setBrush(Qt::blue);
        if (jitter>0)  painter.drawRect(0,this->height()*(1-jitter),7,this->height());
...
void bctl_DblSlider::mousePressEvent(QMouseEvent *event){
    ...
    if (event->buttons()==Qt::LeftButton) ActivePick=1;        // maximum
    else if (event->buttons()==Qt::RightButton) ActivePick=0;  // minimum
    else if (event->buttons()==Qt::MiddleButton) ActivePick=2; // jitter
...
void bctl_DblSlider::tabletEvent(QTabletEvent *event){
 ...
 if (event->type()==QEvent::TabletPress){
        ...
        if (event->pointerType()==QTabletEvent::Eraser) ActivePick=2;
        else ActivePick=1;
}
